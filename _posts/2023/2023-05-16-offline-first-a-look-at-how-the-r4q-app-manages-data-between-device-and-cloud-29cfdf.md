...
tags: [cloud,architecture,mobile]
date: 2023-05-16 03:29:31 
updatedBy: matt
updated: 2026-02-03 02:43:59 
publishedOn: 2026-02-03 02:43:59 
likes: 0
---
...
The application was designed to be used in offline scenarios, using a true 'offline first' capability where media and notes can be added to a case study in an offline state and later synced with the cloud.

Central to this capability was a data broker between online and offline states: the Data Integrity Service (DIS).

What follows are some thoughts about how the DIS works, its original design goals and how it has evolved, its current state and thoughts about how to refactor the service to bring it closer to some of those original design goals that it has moved away from over time.

## Purpose
The purpose of the DIS is to support one of the important goals of the R4Q mobile application to offer true 'offline first' capability. Users will often be offsite and visiting daycare centres, sometimes with little or no internet connectivity.

Evidence they collect and edit on the device in such locations remains on the device only, until it is back online. Only then will the collected evidence be pushed to the cloud.
...
The service should run frequently, perform an appropriate amount of work and then stop work gracefully. If run infrequently, it would consume a lot of resources (time, bandwidth, device battery) to move a lot of data between the device and cloud.
...
This resulted in the original design pattern no longer being appropriate and the service becoming convoluted and bloated.
...
Two ideas that have been raised:
...
Then there is no need to constantly calculate. Just lookup and compare local and Azure values at DIS run-time.
